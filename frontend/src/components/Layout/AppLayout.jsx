...
export default function AppLayout({ children }) {
  ...
  const adminMenuItems = [
    ...
    { key: '/audit-logs', icon: <HistoryOutlined />, label: 'Audit & Access log' },
  ...
  const displayName = user?.name || user?.fullName || user?.email || 'Người dùng';
  const displayEmail = user?.email || 'user@company.com';
  const displayDepartment = user?.departmentName || user?.department || user?.departmentCode || 'Phòng Kỹ thuật';
  const displayRole = user?.role || 'USER';
  const userInitial = displayName.trim().charAt(0).toUpperCase() || 'U';
  const userMenuLinks = [
    { key: 'search', label: 'Tìm kiếm', icon: <SearchOutlined />, path: '/' },
    { key: 'dashboard', label: 'Dashboard cá nhân', icon: <DashboardOutlined />, path: '/dashboard' },
    { key: 'my-documents', label: 'Tài liệu của tôi', icon: <FolderOpenOutlined />, path: '/documents' },
    { key: 'my-versions', label: 'Version của tôi', icon: <FileDoneOutlined />, path: '/versions' },
    { key: 'my-categories', label: 'Danh mục của tôi', icon: <TagsOutlined />, path: '/categories-user' },
    { key: 'history', label: 'Lịch sử thao tác', icon: <HistoryOutlined />, path: '/history' },
          ...
          <Flex align="center" justify="space-between" className={styles.headerInner}>
            <Flex align="center" gap={24} className={styles.headerLeft}>
              ...
              <form className={styles.headerSearchForm} onSubmit={handleHeaderSearchSubmit}>
                ...
                />
                {headerSearch && (
                  <button type="button" className={styles.headerSearchClear} onClick={() => setHeaderSearch('')} aria-label="Xóa tìm kiếm">
                    ×
                  </button>
                )}
              </form>
            ...
            <Flex align="center" gap={10} className={styles.headerActions}>
              ...
              {isAdmin ? (
                <>
                  <Avatar size={38} className={styles.userAvatar}>{userInitial}</Avatar>
                  <Button icon={<LogoutOutlined />} className={styles.logoutButton} onClick={handleLogout}>
                    Đăng xuất
                  </Button>
                </>
              ) : (
                  ...
                  dropdownRender={() => (
                    <div className={styles.userDropdown}>
                      <div className={styles.userDropdownHeader}>
                        <Avatar size={42} className={styles.userAvatar}>{userInitial}</Avatar>
                        <div>
                          <div className={styles.userDropdownName}>{displayName}</div>
                          <div className={styles.userDropdownEmail}>{displayEmail}</div>
                          <div className={styles.userDropdownMeta}>{displayDepartment} · {displayRole}</div>
                        </div>
                      </div>
                      <div className={styles.userDropdownSectionTitle}>KHU VỰC NGƯỜI DÙNG</div>
                      <div className={styles.userDropdownMenu}>
                        {userMenuLinks.map((item) => (
                          <button key={item.key} type="button" className={styles.userDropdownItem} onClick={() => navigate(item.path)}>
                            {item.icon}
                            <span>{item.label}</span>
                          </button>
                        ))}
                      </div>
                      <button type="button" className={`${styles.userDropdownItem} ${styles.userDropdownLogout}`} onClick={handleLogout}>
                        <LogoutOutlined />
                        <span>Đăng xuất</span>
                      </button>
                    </div>
                ...
                >
                  <button type="button" className={styles.userPill}>
                    <Avatar size={32} className={styles.userAvatar}>{userInitial}</Avatar>
